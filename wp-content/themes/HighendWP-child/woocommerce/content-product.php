...
//$fabric_values = get_the_terms( $product->id, 'post_author');
// echo "<pre>";print_r($fabric_values);
//foreach ( $fabric_values as $fabric_value ) {
//	  echo $fabric_value->name;
//}

//echo $product->id;
//$authorvalues = get_the_terms( $product->id, 'pa_user_id');
//echo "au=".$authorvalues;
//echo $product->get_attribute( 'pa_user_id' );
//$u=get_post_meta($product->id, 'pa_user_id', true);
//echo "u=".$u;
...
if (!$product->is_in_stock()) {
	...
	if ($product->product_type != 'external') {
		...
		//$hb_add_to_cart = '<a href="javascript:void(0)"'.'rel="nofollow" data-product_id="'.$product->id.'" class="hb-buy-button add_to_cart_button product_type_'.$product->product_type.'">'. $label.'</a>';
...
?>
	...
	<div class="hb-product-meta">
		...
		<div class="hb-woo-product-details">
			<h3><a href="<?php the_permalink();?>" class="product_title"><?php the_title();?></a></h3>
			<?php
				$post_tmp = get_post($post->ID);
				$author_id = $post_tmp->post_author;
				$last_login= get_user_meta( $author_id, 'last_login', true ); 
			?>
			...
			<?php if($last_login!="") { ?>
			 	<a class="sms-btn sms_<?php echo $post->ID; ?>" id="<?php echo $post->ID; ?>" title="<?php the_title();?>"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/chat_green.png"></a>
				<a class="call-btn call_<?php echo $post->ID; ?>" id="<?php echo $post->ID; ?>" title="<?php the_title();?>"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/phone_green.png"></a>
				<a class="webrtc-btn webrtc_<?php echo $post->ID; ?>" id="<?php echo $post->ID; ?>"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/video_green.png"></a>

			 <?php }else { ?>
				<a class="sms-btn sms_<?php echo $post->ID; ?>" id="<?php echo $post->ID; ?>" title="<?php the_title();?>"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/chat_grey.png"></a>
				<a class="call-btn call_<?php echo $post->ID; ?>" id="<?php echo $post->ID; ?>" title="<?php the_title();?>"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/phone_grey.png"></a>
				<a class="webrtc-btn webrtc_<?php echo $post->ID; ?>" id="<?php echo $post->ID; ?>"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/video_grey.png"></a>
			 <?php } ?>
